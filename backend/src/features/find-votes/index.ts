...
export const findVotes = async (request: FastifyRequest, response: FastifyReply) => {
  ...
  const formattedVotes = foundVotes.map(district => {
    ...
    const votes = district.votes.map(vote => {
      ...
      return {
        ...
        vote_percentage: ((vote.total_vote_count / district.total_vote_count) * 100).toFixed(1),
      }
    })
